...
async function runGauntlet(channel) {
  ...
  // === UGLY BOSS VOTE === const bossCandidates = remaining.sort(() => 0.5 - Math.random()).slice(0, 5);  const bossVoteRow = new ActionRowBuilder().addComponents(   ...bossCandidates.map(p =>     new ButtonBuilder()       .setCustomId(`boss_vote_${p.id}`)       .setLabel(`Vote ${p.username}`)       .setStyle(ButtonStyle.Primary)   ) );  await channel.send({   embeds: [new EmbedBuilder()     .setTitle("👑 UGLY BOSS VOTE")     .setDescription("Vote for who you think should be this game's Ugly Boss.
The winner earns **double $CHARM** if they survive to the podium.")     .setColor(0x9932cc)   ],   components: [bossVoteRow] });  const voteCounts = {}; const alreadyVoted = new Set();  const voteMsg = await channel.send({   content: "🗳️ Voting ends in **15 seconds**. Choose wisely.",   components: [bossVoteRow] });  const voteCollector = voteMsg.createMessageComponentCollector({ time: 15000 });  voteCollector.on('collect', async interaction => {   if (alreadyVoted.has(interaction.user.id)) {     return interaction.reply({ content: '❌ You already voted for the Ugly Boss.', ephemeral: true });   }   alreadyVoted.add(interaction.user.id);   const selectedId = interaction.customId.replace('boss_vote_', '');   voteCounts[selectedId] = (voteCounts[selectedId] || 0) + 1;   await interaction.reply({ content: `✅ Your vote has been cast.`, ephemeral: true }); });  await new Promise(r => setTimeout(r, 15000)); await voteMsg.edit({ components: [] });  const maxVotes = Math.max(...Object.values(voteCounts), 0); const topCandidates = Object.entries(voteCounts)   .filter(([_, count]) => count === maxVotes)   .map(([id]) => id);  const bossId = topCandidates.length   ? topCandidates[Math.floor(Math.random() * topCandidates.length)]   : bossCandidates[Math.floor(Math.random() * bossCandidates.length)].id;  const boss = remaining.find(p => p.id === bossId);  await channel.send(`👹 A foul stench rises... <@${boss.id}> has been chosen as the **UGLY BOSS**! If they make it to the podium, they earn **double $CHARM**...`);
