...
const ZOMBIE_APOCALYPSE_LORE_LINES = [
  ...
  "🩸 {player} finds a red stain and decides panic is now a team sport.",
  "🧠 {player} claims they have a cure plan and it is somehow worse than the zombies.",
  "🪵 {player} drags one wooden plank around like it makes them the mayor of barricades.",
  "🔦 {player} flicks a flashlight into the dark and instantly regrets learning what eyeshine is.",
  "🥫 {player} hoards canned beans like the apocalypse runs on sodium.",
  "💉 {player} waves a mystery syringe and calls it 'science with vibes.'",
  "🚪 {player} slams a door dramatically, which would be more helpful if the door still had hinges.",
  "🧻 {player} mistakes emergency supplies for luxury loot and gets judged by the whole shelter.",
  "🪓 {player} picks up an axe and becomes irresponsible with confidence.",
  "📻 {player} hears static on the radio and treats it like a prophecy.",
  "🧯 {player} carries a fire extinguisher like it solves both infection and personality flaws.",
  "🍕 {player} protects one cold slice of pizza like it is the final artifact of civilization.",
  "🛒 {player} pushes a shopping cart through the outbreak like stealth was never an option.",
  "🧪 {player} mixes random meds and calls it a buff potion.",
  "🩹 {player} puts on one bandage and starts acting clinically immortal.",
  "🪟 {player} boards up a window halfway and announces the shelter is secure enough.",
  "📦 {player} rummages through supply crates with the urgency of a raccoon in a pharmacy.",
  "🧤 {player} wears one glove and calls it infection protocol.",
  "🚨 {player} hears a distant siren and instantly develops worse ideas.",
  "🥤 {player} finds a warm soda and treats it like apocalypse champagne.",
  "🪦 {player} points at a graveyard map and says, 'Let's absolutely not go there.'",
  "🧍 {player} freezes every time someone coughs, which is now often and deeply unhelpful.",
  "🔋 {player} hoards batteries like the flashlight itself will someday grant citizenship.",
  "🧃 {player} drinks expired juice and insists their immune system respects confidence.",
  "🗺️ {player} studies the escape route long enough to become part of the problem.",
  "📣 {player} whispers 'stay quiet' at maximum emotional volume.",
  "🧼 {player} finds hand sanitizer and briefly becomes the cleanest creature in a collapsing world.",
  "🪤 {player} sets a trap, forgets where, and starts walking with caution born of guilt.",
  "🍿 {player} watches the outbreak from a rooftop like this is somehow premium entertainment.",
  "🧯 {player} bangs a pipe once and accidentally schedules everyone's next problem.",
  "😬 {player} says 'it's probably fine' and the shelter immediately feels less stable.",
];

const ZOMBIE_APOCALYPSE_MILESTONE_STAGES = [
  {
    title: "Outbreak Alert - Bad News Running",
    flavor:
      "Sirens scream, group chats die, and the Squigs learn that 'stay calm' is no longer a realistic instruction.",
  },
  {
    title: "First Bite - Panic Becomes Local",
    flavor:
      "Somebody got tagged, nobody agrees how, and now every weird cough sounds like a personal attack.",
  },
  {
    title: "Barricade Hour - Mostly Symbolic",
    flavor:
      "Furniture piles up against doors while confidence leaks out through every crack in the shelter.",
  },
  {
    title: "Supply Run - Outside Was a Mistake",
    flavor:
      "The loot is tempting, the streets are cursed, and every surviving Squig is one snack away from disaster.",
  },
  {
    title: "Fake Cure Season - Science by Vibes",
    flavor:
      "Mystery syringes appear, rumors spread, and nobody alive here should be trusted with a clipboard.",
  },
  {
    title: "Nightfall - Scratchy Sounds Everywhere",
    flavor:
      "The lights dim, the windows rattle, and something outside has started testing the barricades with enthusiasm.",
  },
  {
    title: "Shelter Breach - Teeth at the Door",
    flavor:
      "Boards snap, alarms fail, and the infected stop asking politely to come inside.",
  },
  {
    title: "Final Safehouse - Last Squigs Standing",
    flavor:
      "The cure is fake, the walls are tired, and only a few lucky weirdos remain between survival and full zombie content.",
  },
];

const ZOMBIE_APOCALYPSE_STORIES = {
  hidingSpots: [
    "{victim} hid under a pile of canned food until the whole shelf came down like judgment with a barcode.",
    "Trying to stay quiet in a bathtub, {victim} pulled the curtain too hard and took the whole rod as a final opinion.",
    "{victim} squeezed into an air vent and learned too late that panic breathing has acoustics.",
    "The supply closet looked safe until {victim} got buried under bleach, paper towels, and bad planning.",
    "{victim} hid behind a boarded window just as the boards remembered they were decorative at best.",
    "Inside a shopping cart fort, {victim} discovered that wheels and stability are lifelong enemies.",
    "{victim} crawled into a dumpster for stealth and got compacted by apocalypse-level bad luck.",
    "A pharmacy shelf concealed {victim} right until cough syrup, gravity, and irony finished the job.",
    "{victim} slipped into the back seat of an abandoned car and met the horn with their full body weight.",
    "Trying to hide in a freezer, {victim} was flash-frozen by bad timing and one violent door slam.",
    "{victim} hid in the attic until rotten boards and infected curiosity reached an agreement.",
    "Beneath a collapsed sofa fort, {victim} ran out of oxygen, hope, and structural support at the same time.",
    "{victim} curled up inside a laundry pile and got launched when someone panicked and grabbed the whole hamper.",
    "The rooftop water tank looked safe until {victim} discovered ladders work both ways, especially downward.",
    "{victim} hid under a checkout counter and was trampled by survivors treating canned soup like currency.",
    "Inside a tool chest, {victim} learned that quiet metal boxes become loud coffins when dropped.",
    "{victim} tucked into a school locker and got folded by the apocalypse's worst hide-and-seek result.",
    "Trying to hide behind a fake medical tent, {victim} was flattened when the frame gave up spiritually.",
    "{victim} burrowed into a pile of blankets and got mistaken for emergency insulation.",
    "A church confessional concealed {victim} until the infected pushed the whole booth over with conviction.",
    "{victim} hid inside a lifeguard stand and got kicked loose by chaos, splinters, and beachfront denial.",
    "Beneath the generator tarp, {victim} discovered that fuel leaks and static sparks are a rude pairing.",
    "{victim} wedged into a boarded shop window and got pancaked when the crowd hit the glass together.",
    "Trying to hide in a cereal aisle display, {victim} was avalanched by twelve family-size boxes of consequences.",
    "{victim} slipped into a hospital laundry cart and got rolled directly into outbreak management.",
    "The fake wall panel hid {victim} perfectly until the whole thing popped loose like a budget jump scare.",
    "{victim} camped inside a dog crate and learned the latch only works emotionally.",
    "Behind a vending machine, {victim} was crushed when the shelter tried one last desperate tilt for snacks.",
    "{victim} hid in a church basement pantry until a stack of mason jars chose violence.",
    "Inside a school bus seat compartment, {victim} got folded, jammed, and removed from the survivor census.",
  ],
  clumsiness: [
    "{victim} slipped on spilled hand sanitizer and launched straight into a barricade nail with cinematic commitment.",
    "Trying to sprint from one bite rumor, {victim} tripped over a duffel bag and face-planted into apocalypse history.",
    "{victim} climbed a shaky fence, celebrated too early, and got introduced to the ground in four ugly steps.",
    "One loose floorboard turned {victim}'s brave hallway run into a full-body argument with gravity.",
    "{victim} tried to swing an axe dramatically and discovered walls swing back in their own way.",
    "A panicked turn through the dark sent {victim} straight through a glass door that was trying very hard to be obvious.",
    "{victim} backed into a lit camp stove while giving survival advice nobody had asked for.",
    "Trying to leap a blood slick, {victim} achieved neither the leap nor the rest of the evening.",
    "{victim} tripped on their own emergency rope and got yoinked off a stairwell like homemade prey.",
    "A shopping basket snagged {victim}'s ankle and turned a loot run into a public elimination event.",
    "{victim} misjudged the roof edge by one heroic inch and the apocalypse handled the paperwork.",
    "While checking a bite that was absolutely not there, {victim} walked straight into a boarded window corner.",
    "{victim} dove for a medkit, missed the medkit, and found a much more permanent solution instead.",
    "A loose generator cable turned {victim} into the shelter's least educational cautionary tale.",
    "{victim} tried to kick a zombie hand away from under a door and kicked the door open instead.",
    "Mid-panic reload, {victim} forgot the part where running and stairs are enemies during the end times.",
    "{victim} slipped in the rain, bounced off a barricade, and completed a deeply unhelpful survivor combo.",
    "Trying to slide under a half-shut gate, {victim} forgot they are mostly limbs and bad timing.",
    "{victim} grabbed the wrong shelf during an after-hours pharmacy sprint and brought the aisle down with them.",
    "A fake-cure cooler tipped, {victim} skidded, and the shelter quietly marked them as expired.",
    "{victim} power-walked into the dark with a flashlight off for stealth and found a staircase by theology alone.",
    "While carrying too many batteries, {victim} tripped over one tiny zombie toy and lost the whole timeline.",
    "{victim} tried to vault a hospital gurney and got folded by medicine's final revenge.",
    "One bad pivot around a supply crate sent {victim} into a wall of pipes that had been waiting all outbreak.",
    "{victim} backed away from a groan, stepped on a loose can, and rolled directly into the apocalypse's lap.",
    "Trying to be the hero at the barricade, {victim} leaned too far and got deleted by architecture.",
    "{victim} dropped their snack, lunged for it, and discovered priorities can in fact kill you.",
    "A falling exit sign clocked {victim} mid-sprint, proving the building itself had picked a side.",
    "{victim} tried to outrun panic in flip-flops, which the zombie era considered personally offensive.",
    "While staring at the radio for answers, {victim} walked clean off the loading dock.",
  ],
  sabotage: [
    "{killer} told {victim} the bite was 'probably just a rash' and shoved them back into the crowd.",
    "{killer} locked the barricade one second too early and left {victim} outside with the soundtrack still screaming.",
    "Claiming the alley was clear, {killer} sent {victim} straight into a hungry little surprise party.",
    "{killer} swapped the real medkit for a fake cure cooler and let {victim} science themselves to death.",
    "{killer} yelled 'run left!' with total confidence, which was unfortunate because the horde lived left.",
    "{killer} used {victim} as zombie bait during a supply run and called it team strategy.",
    "After hearing one tiny cough, {killer} framed {victim} as infected and let panic do the rest.",
    "{killer} shoved {victim} into the pharmacy shutters right as they started closing like metal teeth.",
    "{killer} promised the roof ladder was stable, then stopped supporting it emotionally or physically.",
    "{killer} told {victim} to test the fake cure first and watched the consequences go clinical immediately.",
    "{killer} slammed the safehouse door on {victim}'s fingers and let the infected finish the argument.",
    "{killer} threw a can across the room, blamed {victim} for the noise, and let the horde follow the reviews.",
    "{killer} swore the basement was empty, then heard the groaning and simply chose not to update {victim}.",
    "{killer} cut {victim} loose from the escape rope with all the warmth of a bad patch note.",
    "With one ugly shove, {killer} sent {victim} through a brittle barricade and into direct customer service from the undead.",
    "{killer} handed {victim} the unloaded weapon and kept the only good exit path for themselves.",
    "{killer} pointed at {victim}'s sleeve stain and yelled 'BITE!' loud enough to weaponize the whole shelter.",
    "{killer} baited {victim} into opening the supply van, then used the chaos inside as a tactical disposal method.",
    "{killer} promised they would cover {victim}, which turned out to mean with blame.",
    "{killer} kicked the board stack out from under {victim} and let the breach become everyone's problem.",
    "{killer} sent {victim} to turn on the generator knowing full well the fuel line had entered its villain arc.",
    "{killer} whispered that the rooftop evac was here, then watched {victim} sprint toward absolutely nothing.",
    "{killer} framed {victim} as the snack thief and let famine, panic, and mob logic do the rest.",
    "{killer} used {victim} to jam the closing gate and called it 'buying time' with grotesque confidence.",
    "{killer} shoved {victim} toward a zombie wearing hospital scrubs and announced, 'Ask that one for help.'",
    "{killer} swapped the flashlight batteries, let {victim} hit pitch black, and trusted the outbreak to finish strong.",
    "{killer} opened the barricade 'just a little' and used {victim} to measure how bad that idea really was.",
    "{killer} told {victim} the bite scan came back clean, which was bold for someone who made it up on the spot.",
    "{killer} blamed {victim} for the alarm trip and watched the whole shelter vote with fear.",
    "{killer} pushed {victim} into the escape bus doorway and let panic compress the rest of the tragedy.",
  ],
  resurrections: [
    "The bite looked fatal, but somehow {victim} comes back swinging after a deeply suspicious power nap.",
    "A mystery antidote from an unlabeled cooler kicks in, and {victim} is alive for reasons no doctor would sign.",
    "Just when the shelter gives up, {victim} coughs once, sits up, and acts like this was all part of the plan.",
    "The infection stalls out, glitches, and politely returns {victim} to the survivor roster.",
    "A radio preacher yells 'RISE!' at exactly the wrong moment, and {victim} unfortunately does.",
    "A half-baked cure made from gummies, sports drink, and lies somehow restores {victim}.",
    "The fever breaks, the eyes refocus, and {victim} comes back looking slightly haunted but definitely playable.",
    "A lightning flicker hits the shelter, the power returns, and {victim} reboots with it.",
    "After being declared gone, {victim} crawls out from under the gurney and requests a rematch with death.",
    "A zombie bite that should have finished the job instead just made {victim} aggressively hydrated and weirdly alive.",
    "The fake cure turns out to be fake in the helpful direction, and {victim} gets restored on a technicality.",
    "A medkit from 2009 somehow contains exactly what {victim} needed and absolutely should not have worked.",
    "{victim} flatlines emotionally, not medically, then returns once the snack table reopens.",
    "A pile of cold packs, three bandages, and one terrible pep talk bring {victim} back into the timeline.",
    "The horde moves on, the room goes quiet, and {victim} stands back up like buffering finally ended.",
    "A totally unlicensed field treatment clears {victim} just enough to be everyone else's problem again.",
    "{victim} was 'mostly dead,' which in this outbreak apparently still leaves room for sequels.",
    "One emergency flare, one loud sneeze, and {victim} comes back cured enough for now.",
    "The bite mark fades, the panic drops, and {victim} returns with a fresh excuse and zero medical proof.",
    "Somehow not fully dead, not fully infected, and definitely back, {victim} rejoins the run in pure Squig fashion.",
  ],
};
...
const SURVIVAL_REVIVE_FAIL_LORE = {
  ...
  zombie_apocalypse: [
    "🧟 {player} begged the infection to skip them because they were 'working on themselves.' The infection remained unsupportive.",
    "💉 {player} injected a mystery cure found in a gym bag and only achieved premium disappointment.",
    "🩹 {player} wrapped the bite in six bandages and called it legally invisible. The outbreak disagreed.",
    "📻 {player} waited for a radio miracle and got static, dread, and no medical clearance.",
    "🧪 {player} mixed cough syrup with sports drink and called it anti-zombie tech. It mostly tasted purple.",
    "🚪 {player} tried to respawn by dramatically reopening the barricade. The shelter voted no with urgency.",
    "🧠 {player} announced they were 'too funny to turn' and the apocalypse refused to engage with the argument.",
    "🥫 {player} built a small fort out of canned beans hoping immunity respected geometry. It did not.",
    "🔦 {player} flashed a light at the bite like it was a checkout scanner for bad luck.",
    "🧼 {player} used enough sanitizer to qualify as flammable and still remained medically tragic.",
    "🪵 {player} nailed one board to a wall and declared the plague handled. The wall looked embarrassed.",
    "🧯 {player} tried to purge the infection with a fire extinguisher and mostly just seasoned the hallway.",
    "🍕 {player} offered the bite one last pizza slice as a peace treaty. The bite wanted more than carbs.",
    "🗺️ {player} searched the map for a cure zone and only found parking, stairs, and worsening optics.",
    "😬 {player} insisted they were 'probably not turning' while looking exactly like someone who was probably turning."
  ],
};
